...
function Layout() {
  ...
  const nav = [
    ...
    { to: '/contents', icon: FileText, label: t('nav.contents') },
      ...
      <aside className="w-64 bg-gray-900 text-white flex flex-col">
        ...
        <nav className="flex-1 p-3 space-y-1">
          {nav.map(({ to, icon: Icon, label }) => (
            <NavLink key={`${to}-${locale}`} to={to} end={to === '/'} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition ${isActive ? 'bg-violet-600 text-white' : 'text-gray-300 hover:bg-gray-800'}`}>
              <Icon size={18} />{label}
            </NavLink>
          ))}
        </nav>
